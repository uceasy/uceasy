...
class UCEPhylogenomics:
    ...
    def _get_match_contigs_to_probes(self):
        ...
        return self.__processor.match_contigs_to_probes(
            self.__output + "/uce_search_results", self.__contigs, self.__probes
        )
    ...
    def _get_get_match_counts(self):
        ...
        return self.__processor.get_match_counts(
            self.__match_count_output, self.__locus_db, self.__taxon_set_conf, "all"
        )

    def _get_get_fastas_from_match_counts(self):
        self.__alignments = self.__output + "/all-taxa-incomplete.fasta"

        return self.__processor.get_fastas_from_match_counts(
            self.__alignments,
            self.__contigs,
            self.__locus_db,
    ...
    def _get_explode_get_fastas_file(self):
        return self.__processor.explode_get_fastas_file(
            self.__alignments, self.__output + "/exploded_fastas"
        )

    def _get_secap_align(self):
        self.__nexus_files = self.__output + "/nexus_files"

        return self.__processor.seqcap_align(
            self.__nexus_files,
            self.__alignments,
            self.__taxa,
            self.__aligner,
    ...
    def _get_get_gblocks_trimmed_alignments_from_untrimmed(self):
        cmd = self.__processor.get_gblocks_trimmed_alignments_from_untrimmed(
            self.__nexus_files + "_gblocks", self.__nexus_files, self.__log
        )
    ...
    def _get_remove_locus_name_from_nexus_lines(self):
        cmd = self.__processor.remove_locus_name_from_nexus_lines(
            self.__nexus_files + "_clean", self.__nexus_files, self.__log
        )
        self.__nexus_files = self.__nexus_files + "_clean"
        return cmd

    def _get_get_only_loci_with_min_taxa(self):
        cmd = self.__processor.get_only_loci_with_min_taxa(
            self.__nexus_files + "_min_taxa",
            self.__nexus_files,
            self.__taxa,
            self.__percent,
            self.__log,
    ...
    def _get_nexus_files_for_raxml(self):
        cmd = self.__processor.format_nexus_files_for_raxml(
            self.__nexus_files + "_raxml", self.__nexus_files, self.__log, self.__charsets
        )
